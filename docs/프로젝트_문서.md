...
-  **사용자 DB**: 로그인 시 User 저장
-  **비로그인 둘러보기**: 화면 공개, 기능 사용 시 로그인 유도
...
| --- | --- |
| openai/gpt-4o-mini | fal-ai/any-llm + openai/gpt-4o-mini |
| google/gemini-flash-1.5 | fal-ai/any-llm + google/gemini-flash-1.5 |
| fal-ai/flux-2 | fal-ai/flux-2 |
...
FAL_TEXT_DEFAULT_MODEL=openai/gpt-4o-mini
...
- `FAL_TEXT_DEFAULT_MODEL`: 기본 텍스트 모델 (기본값: openai/gpt-4o-mini)
...
- **ChatSession**: `user`, `folder`(nullable), `title`, `messages`(JSON), `model`, `system_instruction`, `recommended_prompts`(JSON), `last_modified`, `created_at`

### Job 모델
- `user` (FK, 사용자 연결), `status` (IN_QUEUE | IN_PROGRESS | COMPLETED | FAILED)
- `provider`, `model`, `arguments`, `result_json`, `error`, `created_at`, `updated_at`
...
##  현재 진행 상황 (2026-01-28 최신)

###  완료된 기능 (로컬/테스트 기준 안정화)

#### 프론트엔드
- [x] React + TypeScript + Vite, 다크/라이트 모드, 채팅·폴더 UI
- [x] **비로그인 둘러보기**: 화면 공개, 기능 사용 시 로그인 유도
- [x] 로그인 후 **폴더·채팅 DB 저장**, 로그아웃 후 재로그인 시 복원
- [x] 이미지/비디오 생성 **Jobs 비동기 + 폴링**
- [x] **이미지 확대 보기 + 다운로드** 모달
- [x] **이미지 수정 모드(기존 이미지 기반 요청)** UI
- [x] 실패 시 **재시도 버튼** 제공

#### 백엔드
- [x] **chats 앱**: Folder, ChatSession 사용자별 CRUD
- [x] **Jobs 비동기**: 사용자당 최대 4건 동시, 429 초과 시
- [x] **AI 라우터**: 텍스트/이미지/비디오 통합 호출
- [x] Firebase 토큰 검증 → JWT 발급

#### 인프라
- [x] Docker Compose, Nginx, PostgreSQL, Redis, MinIO, Celery Worker
- [x] 헬스체크 엔드포인트 및 기본 로그 확인

###  최근 완료 (2026-01-28)

- **비디오 모델**: `fal-ai/sora-2/text-to-video`로 엔드포인트 정리
- **이미지 모델**: `fal-ai/flux-2`, `fal-ai/nano-banana` 정리
- **AI 폴더 생성 안정화**: 플래너 응답 정규화 + 실패 시 롤백
- **UI 개선**: 이미지 확대/다운로드, 실패 재시도 버튼
...
- `docs/빠른_시작_가이드_mac.md`: macOS 로컬 실행 가이드
...
### 2026-01-24 (이전)
-  **사용자 DB**: 커스텀 User (`users.User`), `photo_url`, `last_login_at`
...
-  **비로그인 둘러보기**: ProtectedRoute 제거, 화면 공개, 기능 사용 시 로그인 유도
