...
import { ChevronDown, Box, Video } from 'lucide-react';
...
export const MODELS: AIModel[] = [
  // GPT Series (Mapped to Gemini for functionality)
  { id: 'gpt-5.2-instant', name: 'ChatGPT-5.2 Instant', category: 'GPT', apiModelName: 'gemini-3-flash-preview' },
  { id: 'gpt-5.2-thinking', name: 'ChatGPT-5.2 Thinking', category: 'GPT', apiModelName: 'gemini-3-pro-preview' },
  
  // Gemini Series
  { id: 'gemini-3-pro', name: 'Gemini 3 Pro Preview', category: 'Gemini', apiModelName: 'gemini-3-pro-preview' },
  { id: 'gemini-3-flash', name: 'Gemini 3 Flash Preview', category: 'Gemini', apiModelName: 'gemini-3-flash-preview' },
  
  // Image Series
  { id: 'gpt-image-1', name: 'GPT Image 1', category: 'Image', apiModelName: 'gemini-2.5-flash-image', isImage: true },
  { id: 'nano-banana', name: 'NanoBanana', category: 'Image', apiModelName: 'gemini-2.5-flash-image', isImage: true },
  
  // Video Series
  { id: 'sora', name: 'SORA', category: 'Video', apiModelName: 'veo-3.1-fast-generate-preview', isVideo: true },
...
export const ModelSelector: React.FC<ModelSelectorProps> = ({ selectedModel, onSelect, disabled }) => {
  ...
  const getModelIcon = (modelId: string) => {
    switch (modelId) {
      case 'gpt-5.2-instant': return <OpenAILogo className="w-4 h-4 text-emerald-400" />;
      case 'gpt-5.2-thinking': return <OpenAILogo className="w-4 h-4 text-emerald-500" />;
      
      case 'gemini-3-pro': return <GeminiLogo className="w-4 h-4 text-blue-400" />;
      case 'gemini-3-flash': return <GeminiLogo className="w-4 h-4 text-yellow-400" />;
      
      case 'gpt-image-1': return <OpenAILogo className="w-4 h-4 text-purple-400" />;
