...
import { Menu, MessageSquare, Settings, HelpCircle, SquarePen, LogIn, User as UserIcon, Folder, FolderPlus, X, Type, Wand2, Trash2, ChevronRight, ChevronDown, Video, Image as ImageIcon, FileText, Lightbulb } from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  return (
    ...
    <div 
      className={`fixed top-0 left-0 h-full bg-neutral-900 border-r border-neutral-800 z-50 transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] flex flex-col
    ...
    >
      <div className="flex flex-col w-full px-3 pt-4 gap-2">
        <button 
            onClick={onToggle}
            className="group flex items-center w-full p-3 rounded-xl transition-all duration-300 mb-2
            bg-blue-600 text-white shadow-[0_0_15px_rgba(37,99,235,0.6)] border border-blue-500/50
            hover:bg-blue-500 hover:shadow-[0_0_25px_rgba(37,99,235,0.9)] hover:border-blue-400 hover:scale-[1.02]"
        >
            <div className="flex items-center justify-center w-6 h-6 shrink-0">
                                                ...
                                                <div className="ml-4 pl-2 border-l border-neutral-800 space-y-0.5 mt-1 mb-2 animate-fade-in">
                                                    {chats.map((chat) => (
                                                        <div
                                                            key={chat.id}
                                                            onClick={() => {
                                                                loadChatSession(chat.id, folder.id);
                                                                if(window.innerWidth < 768) onToggle();
                                                            }}
      ...
      <div className="p-3 pb-6 mt-auto flex flex-col gap-1">
         {user ? (
            <div className="mb-2">
                <button 
                  onClick={async () => {
                      try { await signOut(); } catch (e) { alert("로그아웃 실패"); }
                  }}
                  className="group flex items-center w-full p-3 rounded-xl text-neutral-400 hover:text-white hover:bg-neutral-800 transition-colors duration-200"
                >
                   <div className="flex items-center justify-center w-6 h-6 shrink-0">
            ...
            <NavItem icon={LogIn} label="로그인" onClick={async () => {
                try { await signIn(); } catch(e) { alert("로그인 실패"); }
            }} />
         )}
    ...
    {showFolderModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            <div 
                ...
                onClick={resetModal}
            />
            <div className="relative bg-neutral-900 border border-neutral-800 rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden animate-fade-in">
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <textarea 
                                    ...
                                    autoFocus
                                    onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleCreateAI(); } }}
                                />
                            </div>
                            <div className="flex gap-2">
                                <button onClick={() => setFolderStep('selection')} className="flex-1 px-4 py-2.5 rounded-lg text-sm font-medium text-neutral-400 hover:bg-neutral-800 transition-colors">뒤로</button>
