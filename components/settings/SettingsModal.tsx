...
import { X, RefreshCcw, User, Shield, Zap, Sparkles, Plus, Trash2, Layout, BookOpen } from 'lucide-react';
...
export const SettingsModal: React.FC<SettingsModalProps> = ({ 
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            {/* Modal Content */}
            <div className="relative bg-[#0a0a0a] border border-neutral-800 rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden animate-fade-in flex flex-col h-[600px] max-h-[90vh]">
                
                {/* Header */}
                <div className="flex items-center justify-between p-5 border-b border-neutral-800 bg-neutral-900/50 shrink-0">
                    <h3 className="text-white font-medium text-lg flex items-center gap-2">
                        {activeTab === 'profile' ? <User size={20} className="text-neutral-400" /> : <BookOpen size={20} className="text-indigo-400" />}
                        {activeTab === 'profile' ? '설정 및 활동' : '프롬프트 관리'}
                    </h3>
                    <button 
                        onClick={onClose} 
